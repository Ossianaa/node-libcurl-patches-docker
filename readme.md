...
| :---: | :---: | :--- |
| `curl-d755a5f7c009dd63a61b2c745180d8ba937cbfeb.patch` | curl | curl-impersonate base profile; SOCKS5 UDP relay for QUIC; HTTP/3 fingerprint options (`CURLOPT_HTTP3_FP_QUIC/SETTINGS/TRANSPORT_PARAMS/TLS` 331-334) + QUIC brotli cert compression; HTTP/2 PRIORITY frame fix (`h2_pri_spec()` init + per-stream once-only submission — the spec was uninitialized stack → UB, the frame silently vanished on some platforms); Chrome 152 extension fingerprint: `CURLOPT_TLS_EXTENSION_PERMUTATION` default (H2) and H3 default set now include trust_anchors (kExtensions index 27), `CURLOPT_TRUST_ANCHORS` (337) sets the 0xCA34 anchor ID list — hex wire format or tls.peet.ws-style anchor names ("44947.2.14,11129.9.8,..."), default = Chrome 152 sample |
| `boringssl-1c7d52ef3e3f373302cb957089fa783d1e5fd8cd.patch` | boringssl | base patch; per-ctx TLS-level GREASE on/off, configurable ALPS protocol/settings, 0-based decimal extension permutation, trust_anchors (0xCA34) list preserved from `SSL_CTX_set1_requested_trust_anchors` (empty-list default kept for plain libssl users); `SSL_CTX_set_ciphersuites` keeps OpenSSL semantics (colon-separated TLS 1.3 suite names), `SSL_CTX_set_ciphersuites_quic` takes the fingerprint flag digits ("123") and only affects QUIC |
...
Debug aid: setting `CHDUMP=<file>` makes the QUIC TLS path write the raw
ClientHello (first outbound TLS handshake message) to `<file>`, for
fingerprint verification without a live H3 checker (e.g. when
fp.impersonate.pro blocks datacenter IPs).
